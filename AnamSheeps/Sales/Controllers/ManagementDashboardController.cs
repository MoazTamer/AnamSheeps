    ...
    public class ManagementDashboardController : Controller
    {
        ...
        public IActionResult ViewAllDailyMovements(DateTime? date = null)
        {
            ...
            foreach (var movement in data.Movement)
            {
                ...
                var warehouse = data.WarehouseMovement?
                    .FirstOrDefault(x => x.WarehouseMovement_UserID == movementModel.DailyMovement_UserID);

                if (warehouse != null && data.Mortality != null)
                {
                    movementModel.Mortality = data.Mortality
                        .Where(m => m.WarehouseMortality_MovementID == warehouse.WarehouseMovement_ID)
                        .ToList();
                }
                else
                {
                    movementModel.Mortality = new List<ModelWarehouseMortality>();
                }
