...
function attachBalanceAutoCalculation() {
    ...
    $('.quantity-input, .price-input, .total-input, .payment-type, ' +
        ...
        '.supplier-amount-input, .supplier-name-input, .supplier-payment-type, ' +  
        '.customer-amount-input, .customer-name-input, .customer-payment-type, ' +  
...
function addNewSupplierRow() {
    ...
    const newRow = `
        <tr class="supplier-row">
            ...
            <td>
                <select class="form-select form-select-sm supplier-payment-type">
...
function calculateSupplierTotals() {
    ...
    $('#suppliersTableBody tr.supplier-row').each(function () {
        ...
        const paymentType = $(this).find('.supplier-payment-type').val();
        ...
        if (paymentType === "تحويل") transferTotal += amount;
    });

    $('#grandTotalSuppliers').text(grandTotal.toFixed(2));
    $("#cashTotalSuppliers").text(cashTotal.toFixed(2));
    $("#transferTotalSuppliers").text(transferTotal.toFixed(2));
...
function addNewCustomerRow() {
    ...
    const newRow = `
        <tr class="customer-row">
            ...
            <td>
                <select class="form-select form-select-sm customer-payment-type">
                    <option value="نقدا" selected>نقدا</option>
                    <option value="تحويل">تحويل</option>
                </select>
            </td>
...
function calculateCustomerTotals() {
    ...
    $('#customersTableBody tr.customer-row').each(function () {
        ...
        const paymentType = $(this).find('.customer-payment-type').val();
    ...
    $("#cashTotalCustomers").text(cashTotal.toFixed(2));
    $("#transferTotalCustomers").text(transferTotal.toFixed(2));
    calculateBalanceTotals();
    return grandTotal;
}
// Delete customer row
...
function saveMovement() {
    ...
    $('#customersTableBody tr.customer-row').each(function () {
        ...
        customers.push({
            ...
            DailyMovementCustomer_PaymentType: paymentType,
    ...
    $('#suppliersTableBody tr.supplier-row').each(function () {
        ...
        suppliers.push({
            ...
            DailyMovementSupplier_PaymentType: paymentType,
...
$(document).ready(function () {
    ...
    $('#suppliersTableBody tr.supplier-row').each(function () {
        ...
        row.find('.supplier-amount-input, .supplier-payment-type')  
            .off('input change').on('input change', function () {
                calculateSupplierTotals();
                calculateBalanceTotals();
                calculateGrandSummary();
    ...
    $('#customersTableBody tr.customer-row').each(function () {
        ...
        row.find('.customer-amount-input, .customer-payment-type')  
            .off('input change').on('input change', function () {
                calculateCustomerTotals();
                calculateBalanceTotals();
                calculateGrandSummary();
