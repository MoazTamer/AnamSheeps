...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy("Customer_View", policy => policy.RequireClaim("Customer_View", "True"));
    options.AddPolicy("Customer_Create", policy => policy.RequireClaim("Customer_Create", "True"));
    options.AddPolicy("Customer_Edit", policy => policy.RequireClaim("Customer_Edit", "True"));
    options.AddPolicy("Customer_Delete", policy => policy.RequireClaim("Customer_Delete", "True"));

    options.AddPolicy("Supplier_View", policy => policy.RequireClaim("Supplier_View", "True"));
    options.AddPolicy("Supplier_Create", policy => policy.RequireClaim("Supplier_Create", "True"));
    options.AddPolicy("Supplier_Edit", policy => policy.RequireClaim("Supplier_Edit", "True"));
    options.AddPolicy("Supplier_Delete", policy => policy.RequireClaim("Supplier_Delete", "True"));

    options.AddPolicy("Product_View", policy => policy.RequireClaim("Product_View", "True"));
    options.AddPolicy("Product_Create", policy => policy.RequireClaim("Product_Create", "True"));
    options.AddPolicy("Product_Edit", policy => policy.RequireClaim("Product_Edit", "True"));
    options.AddPolicy("Product_Delete", policy => policy.RequireClaim("Product_Delete", "True"));

    options.AddPolicy("ExpenseItem_View", policy => policy.RequireClaim("ExpenseItem_View"));
    options.AddPolicy("ExpenseItem_Create", policy => policy.RequireClaim("ExpenseItem_Create"));
    options.AddPolicy("ExpenseItem_Edit", policy => policy.RequireClaim("ExpenseItem_Edit"));
    options.AddPolicy("ExpenseItem_Delete", policy => policy.RequireClaim("ExpenseItem_Delete"));

    options.AddPolicy("Mandub", policy => policy.RequireClaim("Mandub"));
    options.AddPolicy("Amin", policy => policy.RequireClaim("Amin"));

    options.AddPolicy("SalesReport", policy => policy.RequireClaim("SalesReport"));
    options.AddPolicy("PurchasesReport", policy => policy.RequireClaim("PurchasesReport"));
...
builder.Services.AddDistributedMemoryCache(); // ضروري لـ Session
builder.Services.AddSession(options =>
{
    options.IdleTimeout = TimeSpan.FromDays(30); // مدة الجلسة
...
//  إنشاء الـ Roles الافتراضية
using (var scope = app.Services.CreateScope())
{
    var roleManager = scope.ServiceProvider.GetRequiredService<RoleManager<ApplicationRole>>();

    string[] roleNames = { "مندوب", "أمين مستودع", "إدارة" };
